document.addEventListener("DOMContentLoaded", () => {
    ...
    const resultDiv: HTMLInputElement | null = document.getElementById('result') as HTMLInputElement;
    ...
    if (calculateButton && carbsInput && fatsInput && proteinInput && resultDiv){
        calculateButton.addEventListener('click', ()=> {
            ...
            const proteins: number = parseFloat(proteinInput.value) || 0;

            const totalCalories: number = (carbs * 4) + (fats * 9) + (proteins * 4);
